...
public abstract class JLibPlugin<C extends ConfigLoader> extends JavaPlugin {
    ...
    protected final void checkUpdate(int resourceId) {
        ...
        checker.checkUpdate(o -> {
            switch (o.getType()) {
                case NEW_UPDATE:
                    this.jLogger.log(ChatColor.GOLD + "A new update is available!", JLogger.LogLevel.MINIMAL);
                    this.jLogger.log(ChatColor.GOLD + "Version " + o.getNewVersion() + " (Current: " + this.getDescription().getVersion() + ')', JLogger.LogLevel.MINIMAL);
                    break;
                case UP_TO_DATE:
                    this.jLogger.log(ChatColor.GREEN + "You are up to date!", JLogger.LogLevel.NORMAL);
                    break;
                case ERROR:
                    this.jLogger.log(ChatColor.RED + "An error occured while trying to check for updates on spigotmc.org!", JLogger.LogLevel.MINIMAL);
                    break;
            }
        });
    }
