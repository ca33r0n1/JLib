...
public final class UUIDFetcher {
    ...
    public UUID getUniqueId() throws Exception {
        ...
        return this.getUUID((String) ((Map) new JSONParser().parse(new InputStreamReader(connection.getInputStream()))).get("id"));
    }
     ...
     */
    @Deprecated
    public void getUniqueIdAsync(JavaPlugin plugin, CallbackHandler<UUID> callbackHandler) {
        this.jLogger.deprecation();
        Bukkit.getScheduler().scheduleAsyncDelayedTask(plugin, () -> {
            try {
                HttpURLConnection connection = (HttpURLConnection) new URL("https://api.mojang.com/users/profiles/minecraft/" + this.name).openConnection();
                callbackHandler.callback(this.getUUID((String) ((Map) new JSONParser().parse(new InputStreamReader(connection.getInputStream()))).get("id")));
            } catch (Exception e) {
                e.printStackTrace();
